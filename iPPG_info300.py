...
def decompress_rows(rows: List[Dict[str, Any]]) -> Tuple[List[Dict[str, Any]], int, int, Optional[str]]:
    ...
    for i, (orig, upsampled, row) in enumerate(zip(origin_ppg_data_list, upsampled_data_list, filtered_rows)):
        ...
        processed_entries.append({
            ...
            "createTime": now_utc.strftime("%Y-%m-%d %H:%M:%S.%f"),
            "collectTime_beijing": (ctime.astimezone(timezone(timedelta(hours=8))).strftime("%Y-%m-%d %H:%M:%S") if ctime and ctime.tzinfo else ""),
            "createTime_beijing": now_utc.astimezone(timezone(timedelta(hours=8))).strftime("%Y-%m-%d %H:%M:%S"),
...
def import_csv_to_mysql(csv_path: str, cfg: Dict[str, Any]) -> bool:
    conn = None
    try:
        ...
        table = cfg["table"]
        risk_col = "sys_stroke_risk_score"
        try:
            with conn.cursor() as cur:
                cur.execute(
                    "SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA=%s AND TABLE_NAME=%s AND COLUMN_NAME IN ('sys_stroke_risk_score','sys_stroker_risk_score')",
                    (cfg["database"], table),
                )
                rows = [r[0] if isinstance(r, tuple) else r.get("COLUMN_NAME") for r in (cur.fetchall() or [])]
                if "sys_stroke_risk_score" in rows:
                    risk_col = "sys_stroke_risk_score"
                elif "sys_stroker_risk_score" in rows:
                    risk_col = "sys_stroker_risk_score"
        except Exception:
            pass
        cols = (
            "device_id, wear_user_id, source_date, @create_time, heart_status, spine_status, lymph_status, @pericardium_status, small_intestine_status, lung_status, stomach_status, sys_mi_risk_score, @sys_stroker_risk_score, mi_risk_level2, stroke_risk_level2"
        )
        sql = (
            f"LOAD DATA LOCAL INFILE %s INTO TABLE `{table}` CHARACTER SET utf8mb4 "
            f"FIELDS TERMINATED BY ',' ENCLOSED BY '""' ESCAPED BY '' "
            f"LINES TERMINATED BY '\r\n' IGNORE 1 LINES ({cols}) "
            f"SET create_time=@create_time, {risk_col}=@sys_stroker_risk_score, pericardium_status=@pericardium_status"
        )
        with conn.cursor() as cur:
            ...
            try:
                ...
                r = cur.fetchone()
                load_cnt = (r[0] if isinstance(r, tuple) else (list(r.values())[0] if isinstance(r, dict) and r else 0)) if r is not None else 0
            except Exception:
                load_cnt = None
        if load_cnt is not None and load_cnt > 0:
            conn.commit()
            logging.info(f"导入CSV到MySQL成功: {csv_path} 插入行数={load_cnt}")
            return True
        raise Exception("LOAD DATA 未插入任何行")
    except Exception as e:
        ...
        try:
            ...
            mode = os.environ.get("CSV_INSERT_MODE", "upsert")
            if mode == "ignore":
                insert_sql = f"INSERT IGNORE INTO `{cfg['table']}` ({','.join(db_cols)}) VALUES ({placeholders})"
                upsert_tail = None
            elif mode == "upsert":
                ...
                upsert_tail = (
                    ...
                    + ",".join([
                        "wear_user_id=VALUES(wear_user_id)",
                        "create_time=VALUES(create_time)",
                        "heart_status=VALUES(heart_status)",
                        "spine_status=VALUES(spine_status)",
                        "lymph_status=VALUES(lymph_status)",
                        "pericardium_status=VALUES(pericardium_status)",
                        "small_intestine_status=VALUES(small_intestine_status)",
                        "lung_status=VALUES(lung_status)",
                        "stomach_status=VALUES(stomach_status)",
                        "sys_mi_risk_score=VALUES(sys_mi_risk_score)",
                        "sys_stroker_risk_score=VALUES(sys_stroker_risk_score)",
                        "mi_risk_level2=VALUES(mi_risk_level2)",
                        "stroke_risk_level2=VALUES(stroke_risk_level2)"
                    ])
                )
            else:
                insert_sql = f"INSERT INTO `{cfg['table']}` ({','.join(db_cols)}) VALUES ({placeholders})"
            ...
            with open(csv_path, "r", encoding="utf-8-sig") as f:
                ...
                with conn.cursor() as cur:
                    for r in reader:
                        risk_val = r.get("sys_stroker_risk_score")
                        vals = [
                            r.get("device_id"),
                            r.get("wear_user_id"),
                            r.get("source_date"),
                        ...
                        if len(batch_vals) >= batch_sz:
                            q = insert_sql + (upsert_tail or "")
                            cur.executemany(q, batch_vals)
                    ...
                    if batch_vals:
                        ...
                        total += len(batch_vals)
            conn.commit()
            logging.info(f"逐行插入导入成功: {total} 行 模式={mode}")
            return total > 0
...
def main() -> None:
    ...
    if os.environ.get("STREAM_ALL", "1") == "0":
        ...
        try:
            for doc in cursor:
                ...
                if did != current_id:
                    ...
                    batch.append(out_path)
                    processed_devices += 1
                    if len(batch) >= int(os.environ.get("DEVICE_BATCH_SIZE", "300")):
        ...
        chunk_size = int(os.environ.get("DEVICE_BATCH_SIZE", "300"))
        for i in range(0, len(device_ids), chunk_size):
            chunk_ids = device_ids[i:i+chunk_size]
            chunk_paths = process_devices_parallel(coll, chunk_ids, start_utc, end_utc, analysis_dir)
            if chunk_paths:
                if os.environ.get("SKIP_ANALYSIS", "0") != "1":
                    logging.info("处理批次分析")
                    process_batch(chunk_paths, OUTPUT_DIR, analysis_dir)
                    clear_output_dir()
    if batch:
        if os.environ.get("SKIP_ANALYSIS", "0") != "1":
            logging.info("处理批次分析")
    ...
    if os.environ.get("RUN_CSV_EXPORT", "1") == "1":
        ...
        try:
            if csv_out and os.path.isfile(csv_out):
                ok = import_csv_to_mysql(csv_out, get_mysql_cfg())
                if ok:
                    logging.info("CSV导入MySQL完成")
                else:
                    logging.error("CSV导入MySQL失败")
            else:
